...
app.get('/special',(req,res) => {
        db.collection('special').find().toArray((err,result) =>{
            if (err) throw err;
            res.send(result);
    })    
})
...
app.get('/shop',(req,res) => {
    ...
    if(stateId && mealId){
      ...
      db.collection('shop').find(query).toArray((err,result) => {
        if(err) throw err;
        res.send(result)
    })
})
...
app.get(`/filter/:mealId`,(req,res) => {
    ...
    db.collection('shop').find(query).sort(sort).toArray((err,result) => {
      if(err) throw err;
      res.send(result)
    })
  })
  ...
  app.get('/details/:id',(req,res) => {
    ...
    db.collection('shop').find({restaurant_id:id}).toArray((err,result) => {
      if(err) throw err;
      res.send(result)
    })
  })
